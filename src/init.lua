...
local function fetchProfileStore()
	local version = game.PlaceId == _G.PLAYERDATAMANAGER_CONFIGURATION.TESTING_PLACE_ID
			and _G.PLAYERDATAMANAGER_CONFIGURATION.TESTING_DATA_VERSION
		or _G.PLAYERDATAMANAGER_CONFIGURATION.DATA_VERSION

	PlayerDataManager._profile_store = ProfileService.GetProfileStore(
		"PlayerData-" .. version,
		PlayerDataManager:_getDefaultData()
	)
...
function PlayerDataManager:_getRandomUserIdForPlayer(player: Player)
	if self._generated_user_ids[player] then
		return self._generated_user_ids[player]
	end
...
function PlayerDataManager:Init(configuration: Configuration)
	if _G.PLAYERDATAMANAGER_INITIALIZED then
		return
	end
	if not configuration then
		return error("No configuration provided for PlayerDataManager:Init")
	end
...
function PlayerDataManager:GetPlayerData(userId: number | string)
	if not _G.PLAYERDATAMANAGER_INITIALIZED then
		return
	end
	...
	if self._loading[userId] then
		repeat
			task.wait()
		until not self._loading[userId]
		return self._loaded[userId]
	end
...
function PlayerDataManager:ViewPlayerData(userId: number | string, version: string?)
	...
	if self._view_only_loading[userId] then
		repeat
			task.wait()
		until not self._view_only_loading[userId]
		return self._view_only_loaded[userId]
	end
...
function PlayerDataManager:GetGlobalLeaderboard(dataKey: string)
	...
	if #key > 50 then
		error(
			"Maximum key length is 50 characters for DataStore name. "
				.. dataKey
				.. " is "
				.. #key
				.. " characters long."
